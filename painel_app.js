...
function grafBarrasDuplo(alvo, rotulos, series){
  // series: [{nome, cor, vals}] — duas séries de barras lado a lado por rótulo
  const W = 640, H = 225, mL = 10, mR = 10, mT = 40, mB = 28;
  const todos = series.flatMap(sr => sr.vals).filter(v => v != null);
  if(!todos.length){ document.getElementById(alvo).innerHTML = "<p class='suave'>Sem dados.</p>"; return; }
  const max = Math.max(...todos, 1);
  const n = rotulos.length, gw = (W - mL - mR) / n;
  let s = `<svg class="graf" viewBox="0 0 ${W} ${H}">`;
  let lx = mL;
  series.forEach(sr => {
    s += `<rect x="${lx}" y="6" width="12" height="12" rx="3" fill="${sr.cor}"/>`;
    s += `<text x="${lx + 17}" y="16.5" font-size="12.5" font-weight="700" fill="#1D3229">${sr.nome}</text>`;
    lx += 17 + sr.nome.length * 7.5 + 22;
  });
  rotulos.forEach((r, i) => {
    series.forEach((sr, j) => {
      const v = sr.vals[i];
      ...
      const h = (H - mT - mB) * v / max;
      const bw = gw * 0.30;
      const x = mL + i * gw + gw * 0.16 + j * (bw + gw * 0.07);
      const y = H - mB - h;
      s += `<rect x="${x.toFixed(1)}" y="${y.toFixed(1)}" width="${bw.toFixed(1)}" height="${h.toFixed(1)}" rx="4" fill="${sr.cor}"/>`;
      s += `<text x="${(x + bw/2).toFixed(1)}" y="${(y - 6).toFixed(1)}" text-anchor="middle" font-size="10.5" font-weight="700" fill="#1D3229">${Math.round(v)}</text>`;
    });
    s += `<text x="${(mL + i*gw + gw/2).toFixed(1)}" y="${H - 9}" text-anchor="middle" font-size="12.5" fill="#4E6A5C">${r}</text>`;
  });
  s += "</svg>";
  document.getElementById(alvo).innerHTML = s;
}
...
function desenhaDossie(){
  ...
  <div class="dossie-cards">
    ...
    <div class="card"><h3>Nascidos vivos ocorridos, ${ANOS[0]} a ${ANOS[NT-1]}</h3><div id="dNV"></div><p class="fonte">${FONTE_SINASC}</p></div>
    <div class="card"><h3>Evolução comparada com o Brasil (${ANOS[0]} = 100)</h3><div id="dNVBrasil"></div><p class="fonte">${FONTE_SINASC} Índice com base 100 em ${ANOS[0]}: as barras mostram a trajetória da unidade e do país na mesma escala. Brasil = soma dos nascidos vivos das 27 UFs.</p></div>
    <div class="full">
  ...
  // evolução comparada unidade × Brasil, indexada no primeiro ano
  const _nvU = ANOS.map((_, a) => mt.blocos.nv_territorio[0][1][a]);
  const _nvB = ANOS.map((_, a) => nvBrasilAno(a));
  grafBarrasDuplo("dNVBrasil", ANOS, [
    {nome:"Unidade", cor:"#279261", vals:_nvU.map(v => v != null && _nvU[0] ? 100*v/_nvU[0] : null)},
    {nome:"Brasil",  cor:"#0A213D", vals:_nvB.map(v => v != null && _nvB[0] ? 100*v/_nvB[0] : null)}
  ]);
...
const FICHAS_EXTRAS = [
  ...
  {rot:"Evolução comparada com o Brasil, 2019 = 100 (dossiê)",
   desc:"Trajetória do volume de nascimentos da unidade e do país na mesma escala: cada série é dividida pelo próprio valor de 2019 e multiplicada por 100. Valores acima de 100 indicam crescimento em relação a 2019; abaixo, queda.",
   n:"Nascidos vivos do ano (da unidade, ou do Brasil pela soma das 27 UFs).",
   d:"Nascidos vivos de 2019 da mesma série (unidade ou Brasil).",
   fonte:"Ministério da Saúde - SINASC, 2019 a 2025."}
];
